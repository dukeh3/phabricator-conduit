...
public class ConduitModuleTest extends ModuleTestCase {
	public void testPingPass() throws Exception {
		expect(connection.call("conduit.ping")).andReturn(new JsonPrimitive("foo")).once();
		...
		final ConduitModule.PingResult expected = new ConduitModule.PingResult("foo");
	...
	public void testConnectPass() throws Exception {
		...
		expect(connection.call(eq("conduit.connect"), capture(paramsCapture))).andReturn(ret).once();

		replayMocks();

		final ConduitModule module = getModule();
		final ConduitModule.ConnectResult result = module.connect("userFoo", null);

		final Map<String, Object> params = paramsCapture.getValue();
		assertEquals("Usernames do not match", "userFoo", params.get("user"));

		final ConduitModule.ConnectResult expected = new ConduitModule.ConnectResult(42, "sessionKeyFoo", "userFoo");
	...
	public void testConduitConnectConnectionFail() throws Exception {
		...
		expect(connection.call(eq("conduit.connect"), capture(paramsCapture))).andThrow(conduitException).once();
	...
	public void testGetCapabilitiesPass() throws Exception {
		...
		expect(connection.call(eq("conduit.getcapabilities"), capture(paramsCapture))).andReturn(ret).once();
	...
	public void testGetCertificatePass() throws Exception {
		...
		expect(connection.call(eq("conduit.getcertificate"), capture(paramsCapture))).andReturn(ret).once();

		replayMocks();

		final ConduitModule module = getModule();
		final ConduitModule.GetCertificateResult result = module.getCertificate("tokenFoo", "hostBar");

		final Map<String, Object> params = paramsCapture.getValue();
		assertEquals("Tokens do not match", "tokenFoo", params.get("token"));
		assertEquals("Hostnames do not match", "hostBar", params.get("host"));

		final ConduitModule.GetCertificateResult expected = new ConduitModule.GetCertificateResult("userFoo",
				"certBar");
		assertEquals("Results do not match", expected, result);
	}
	...
	public void testQueryPass() throws Exception {
		...
		expect(connection.call(eq("conduit.query"), capture(paramsCapture))).andReturn(ret).once();
		...
		expected.put("foo", new ConduitModule.SingleQueryResult("service Foo", expectedFooParams, "return foo"));
		expected.put("bar",
				new ConduitModule.SingleQueryResult("service Bar", new HashMap<String, String>(), "return bar"));
		assertEquals("Results do not match", expected, result);
	}
